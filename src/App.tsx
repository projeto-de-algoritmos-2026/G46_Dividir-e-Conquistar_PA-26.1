...
function App() {
  ...
  useEffect(() => {
    ...
    if (isPlaying && chaosLevel !== -1) {
      interval = setInterval(() => setTimeElapsed(prev => prev + 1), 1000);
    }
    return () => clearInterval(interval);
  }, [isPlaying, chaosLevel]);

  // NOVO: Checagem de Vitória Global
  useEffect(() => {
    if (chaosLevel === 0 && energyStable) {
      setIsPlaying(false);
      setGameWon(true);
    }
  }, [chaosLevel, energyStable]);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8 font-sans flex flex-col items-center relative">
      
      {/* HUD Superior Atualizada com Cronômetro */}
      <header className="w-full max-w-5xl flex justify-between items-end border-b border-slate-800 pb-4 mb-8">
        ...
        <div className="flex items-center gap-8">
          <div className="flex flex-col items-end">
            <span className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-1">Tempo de Operação</span>
            <span className={`text-3xl font-mono font-black ${gameWon ? 'text-emerald-500' : 'text-slate-200'}`}>
              {formatTime(timeElapsed)}
            </span>
          </div>
          <button onClick={handleRestartAll} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-orange-500 border border-orange-500/30 rounded font-bold transition-all cursor-pointer">
            ↻ Reiniciar Sistemas
          </button>
        </div>
      </header>
      ...
      <main className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-2 gap-8 relative z-10">
        ...
        <section className="bg-slate-900 border-2 border-slate-800 rounded-xl p-6 relative shadow-2xl flex flex-col">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className={`text-2xl font-black font-mono ${chaosLevel === 0 ? 'text-emerald-500' : 'text-red-500'}`}>{chaosLevel === -1 ? 0 : chaosLevel}</span>
            </div>
          </div>
          <div className="relative flex-grow h-48 bg-slate-950 rounded-lg border-y-4 border-slate-700 flex flex-wrap content-center justify-center gap-2 p-4">
            {boxes.map((box, index) => (
              <div
                ...
                className={`relative z-10 w-16 h-16 flex flex-col items-center justify-center rounded-lg cursor-pointer transition-all duration-300 select-none
        ...
        <section className="bg-slate-900 border-2 border-slate-800 rounded-xl p-6 relative shadow-2xl flex flex-col">
          ...
          <p className="text-xs text-slate-400 mb-2 font-mono">Conecte os dois geradores MAIS PRÓXIMOS para isolar a rede.</p>
          <div className="relative flex-grow h-64 bg-slate-950 rounded-lg border border-slate-700 overflow-hidden">
            ...
            {energyStable && closestPair && (
              <svg className="absolute inset-0 w-full h-full pointer-events-none">
                <line x1={`${closestPair[0].x}%`} y1={`${closestPair[0].y}%`} x2={`${closestPair[1].x}%`} y2={`${closestPair[1].y}%`} stroke="#10b981" strokeWidth="3" className="drop-shadow-[0_0_5px_#10b981] animate-pulse" />
              </svg>
            )}
            {energyStable && (
      ...
      {gameWon && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-50 flex flex-col items-center justify-center animate-in fade-in duration-500">
          <h2 className="text-7xl font-black text-emerald-500 mb-4 drop-shadow-[0_0_30px_#10b981]">OPERAÇÃO CONCLUÍDA</h2>
          <p className="text-2xl text-slate-300 font-mono mb-8">Fábrica estabilizada em <span className="text-orange-500 font-black">{formatTime(timeElapsed)}</span></p>
          <button onClick={handleRestartAll} className="px-8 py-4 bg-orange-600 hover:bg-orange-500 text-white text-xl font-bold rounded shadow-[0_0_20px_#f97316] transition-all cursor-pointer">
            Iniciar Novo Turno
          </button>
        </div>
      )}
